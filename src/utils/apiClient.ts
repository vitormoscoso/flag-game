...
function getText(
  ...
  if(data?.[0]?.name.nativeName){
    ...
    commonNative =
      data?.[0]?.name.nativeName[nativeNameKey].common;
  }
  let currencyName = "";
  let currencySymbol = ""
...
// return ""
  return `${data?.[0]?.translations?.por?.common}, conhecido originalmente como ${commonNative}, é uma nação localizada na região ${translatedRegions[data?.[0]?.subregion]} e conta com uma população de aproximadamente ${formattedPop}. Possui uma área total de ${formattedArea} km² e sua capital é ${data?.[0]?.capital?.[0]}, além de sua moeda oficial ser o(a) ${currencyName} (${currencySymbol})`;
  }
...
export default function useAxios(currentPage: number, countries: string[], allCountries: string[]) {
      ...
      try {
        ...
        for (let i = 0; i < 3; i++) {
          const randomIndex = Math.floor(
            Math.random() * allCountries.length,
          );
